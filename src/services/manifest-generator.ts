 ...
 * Manifest Generator
 * 
 * Generates Merkle-trie manifests from file collections with signing and verification.
 ...
 * - Rumsfeld scores for knowledge state tracking
 */
...
export interface ManifestCatalogEntry {
  hash: string;
  path: string;
  type?: 'markdown' | 'canvas' | 'metadata' | 'jsonl';
}
...
export class ManifestGenerator {
  ...
  private getFileType(filePath: string): 'markdown' | 'canvas' | 'metadata' | 'jsonl' {
