...
export const canvaslTags = {
  canvaslDirective: 'meta',
  canvaslReference: 'variableName',
  canvaslDimension: 'number',
  canvaslType: 'typeName',
  canvaslEdgeType: 'typeName',
  canvaslR5RSFunction: 'function(variableName)',
  canvaslSchemeExpression: 'string',
};
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (entry.id) {
            ...
            let directiveMatch;
            while ((directiveMatch = directiveRegex.exec(line)) !== null) {
              if (directiveMatch.index !== undefined) {
                const dirStart = currentPos + directiveMatch.index;
                const dirEnd = dirStart + directiveMatch[0].length;
                decorations.push(
                  Decoration.mark({
