...
import type { Canvas } from 'meta-log-db';
import type { AutomatonState, Transition, VerticalTransition, CanvasObject } from '../types/automaton-state.js';
...
class AdvancedSelfReferencingAutomaton {
  ...
  private objectPool = new ObjectPool<CanvasObject>(
    ...
    (obj) => {
      ...
      obj.selfReference = undefined as any;
   ...
   */
  private flattenCanvas(canvas: Canvas): CanvasObject[] {
    const objects: CanvasObject[] = [];
    
    // Add nodes
    if (canvas.nodes) {
      objects.push(...canvas.nodes as CanvasObject[]);
    }
    
    // Add edges
    if (canvas.edges) {
      objects.push(...canvas.edges as CanvasObject[]);
    }
    
    // Add other object types
    for (const [key, value] of Object.entries(canvas)) {
      if (key !== 'nodes' && key !== 'edges' && Array.isArray(value)) {
        objects.push(...value as CanvasObject[]);
      }
    }
...
(lambda (system) 
  ...
  private async executeSelfIO(): Promise<void> {
    await this.load();
